  ...
  + 容量需求:
   + 每分鐘儲存一個檔案:
     ...
     + 8[ch]1000[Hz]24[bit]:
       ...
       + 每日容量~=1980[MB]~=2[GB]。
       + 使用標示512[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(512*0.9)/2=230天。
       + 使用標示1[TB]=931[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(931*0.9)/2=418天(超過1年)。
     + 8[ch]100[Hz]24[bit]:
       + 每分鐘容量=(8*100*3*60)+1024=145024[Bytes]~=142[KB]~=0.14[MB]。
       + 每日容量~=202[MB]。
       + 使用標示512[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(512*0.9)/0.202=2281天(超過6年)。
       + 使用標示1[TB]=931[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(931*0.9)/0.202=4148天(超過11年)。
     + 64[ch]10[Hz]24[bit]:
       + 每分鐘容量=(64*10*3*60)+1024=116224[Bytes]~=114[KB]~=0.112[MB]。
       + 每日容量~=162[MB]。
       + 使用標示512[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(512*0.9)/0.162=2844天(超過7年)。
       + 使用標示1[TB]=931[GB]的SD卡，建議只使用90%的可用容量避免讀寫效率下降，約可使用(931*0.9)/0.202=4148天(超過11年)。
